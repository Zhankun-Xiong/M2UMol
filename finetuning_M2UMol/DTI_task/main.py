...
def main():
    ...
    set_seed(args.seed)#cfg.SOLVER.SEED
    ...
    ignored_params1 = list(map(id, model.drug_extractor.parameters()))

    base_params = filter(lambda p: id(p) not in ignored_params1, model.parameters())
    if cfg.DA.USE:
        ...
        opt = torch.optim.Adam([
            {'params': model.drug_extractor.parameters(), 'lr': 5e-4,'weight_decay':1e-9}, #cfg.SOLVER.LR
            {'params': base_params, 'lr': cfg.SOLVER.LR}, ], lr=cfg.SOLVER.LR)
