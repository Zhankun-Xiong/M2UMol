...
class M2UMol(nn.Module):
    def __init__(self, args):
        ...
        attn_kwargs = {'dropout': args.attdropout}
        ...
        self.args=args
        self.attnfea = torch.nn.MultiheadAttention(128, 4, batch_first=True,dropout=args.attdropout)
        self.mlp1 = nn.ModuleList([nn.Linear(256, 256),
                                   nn.ELU(),
    ...
    def forward(self, idx, alltwograph, alltwographt):
        alltwograph = self.transform(alltwograph)
        alltwographt = self.transform(alltwographt)
        ...
        for conv in self.encoder2D:
            x,_ = conv(x, h_data_2d.edge_index, h_data_2d.batch, edge_attr=edge_attr)
        feats2dt = global_add_pool(x, h_data_2d.batch)
        z1t = self.norm(feats2dt)


############################ Using the pre-trained adapter
        ...
        z3dt = self.linto3d(feats2dt)
        z3dt = self.norm(z3dt)
        z3dt = torch.unsqueeze(z3dt, dim=1)
        zwenbent = self.lintotext(feats2dt)
        zwenbent = self.norm(zwenbent)
        zwenbent = torch.unsqueeze(zwenbent, dim=1)
        zfingert = self.lintobio(feats2dt)
        zfingert = self.norm(zfingert)
        zfingert = torch.unsqueeze(zfingert, dim=1)

        ############################ Using the multi-head attention
        z1 = torch.unsqueeze(z1, dim=1)
        z11 = torch.cat((z3d, zwenben, zfinger), dim=1)
        zl, _ = self.attnfea(z1, z11, z11, average_attn_weights=True)
        zl = z1 + zl

        z1t = torch.unsqueeze(z1t, dim=1)
        z22 = torch.cat((z3dt, zwenbent, zfingert), dim=1)
        zr, _ = self.attnfea(z1t, z22, z22, average_attn_weights=True)
        zr = z1t + zr

        ############################ DDI prediction
        zl = torch.squeeze(zl)
        zr = torch.squeeze(zr)
        zall = torch.cat((zl, zr), dim=1)
        zall = self.MLP(zall, 7)
        predict = zall
        return predict
