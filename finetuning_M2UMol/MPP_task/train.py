...
if __name__ == '__main__':
    ...
    from instantiation import Create_model_ddi_2d
    model_ddi, optimizer_ddi = Create_model_ddi_2d(args,num_tasks)
    model=model_ddi

    model_dict = model.state_dict()
    pretrained_dict=torch.load(args.pretrained_file)
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict)##
    print('pretrain load true 3 layer success')
    ...
    with open(args.result_file, 'a') as f:
        f.write('\n')
        f.write('\n')
        f.write(str(args.batch_size) + '_' + str(args.mweight_decay) + '_' + str(args.mdropout) + '_' + str(args.mattdropout) + '_' + str(args.mlr) + '_' + str(args.jingtiaolr))
        f.write('\n')
        f.close()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_roc > best_val_roc:
            ...
            if not args.output_model_dir == '':
                output_model_path = join(args.output_model_dir, args.dataset, 'model_best.pth')
                saved_model_dict = {
                    'model': model.state_dict()
                }
                torch.save(saved_model_dict, output_model_path)

                filename = join(args.output_model_dir, args.dataset, 'evaluation_best.pth')
                np.savez(filename, val_target=val_target, val_pred=val_pred,
                         test_target=test_target, test_pred=test_pred)

    print(
        'best train: {:.6f}\tval: {:.6f}\ttest: {:.6f}'.format(train_roc_list[best_val_idx], val_roc_list[best_val_idx],
                                                               test_roc_list[best_val_idx]))
    with open(args.result_file, 'a') as f:
        f.write('{:.6f}\t{:.6f}\t{:.6f}'.format(train_roc_list[best_val_idx], val_roc_list[best_val_idx],
                                                               test_roc_list[best_val_idx]))
        f.write('\n')
    if args.output_model_dir is not '':
        output_model_path = join(args.output_model_dir, args.dataset, 'model_final.pth')
        saved_model_dict = {
            'model': model.state_dict()
        }
        torch.save(saved_model_dict, output_model_path)
